...
#if defined(SK_OS_windows)
	using SK_Communication_onRequest = std::function<void(const SK_Communication_Config& config, SK_Communication_handlePacket_Response_IPC_CB ipcResponseCallback, void* nullArg)>;
#elif defined(SK_OS_apple)
...
using SK_ForwardPacketToModule_CB = std::function<void(const SK_String& module, const SK_String& operation, nlohmann::json& payload, SK_Communication_Response& respondWith)>;
