...
public:
    ...
    SK_String(const std::vector<char>& chars) : data(chars.data(), chars.size()) {}
    ...
    #if defined(SK_OS_apple)
        #ifdef __OBJC__
            SK_String(NSString* nsString) {
                if (nsString) {
                    const char* cStr = [nsString UTF8String];
                    if (cStr) {
                        data = std::string(cStr);
                    } else {
                        // Handle the case where UTF8String returns NULL
                        data.clear();
                    }
                } else {
                    // Handle the case where nsString is NULL
                    data.clear();
                }
            }
            ...
            operator NSURL*() const {
                NSString *str = [NSString stringWithCString:data.c_str() encoding:NSUTF8StringEncoding];
                NSURL *url = [NSURL URLWithString:str];
                if (!url) {
                    // Handle invalid URL string case
                    // For example, return a default URL or handle the error appropriately
                    // Here, we'll return a default URL
                    url = [NSURL URLWithString:@"https://default.url"];
                }
                return url;
            }
    
            SK_String& operator=(NSString* other) {
                if (other) {
                    const char* cStr = [other UTF8String];
                    if (cStr) {
                        data = std::string(cStr);
                    } else {
                        // Handle the case where UTF8String returns NULL
