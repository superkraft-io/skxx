...
class SK_Colors {
public:
    static inline std::unordered_map<std::string, SK_Color> list;
};
...
public:
    ...
    std::optional<SK_Color> parse(const SK_String& color) {
    ...
    SK_Color_Format detectColorFormat(const std::string& color) {
        // Hex color detection
        ...
        if (SK_Colors::list.find(color) != SK_Colors::list.end()) {
    ...
    std::optional<SK_Color> parseHex(const std::string& color) {
        std::regex hexRegex("^#([0-9a-fA-F]{3,4}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})$");
    ...
    std::optional<SK_Color> parseRGB(const std::string& color) {
        std::regex rgbRegex("^rgb\\(\\s*(\\d+)\\s*,?\\s*(\\d+)\\s*,?\\s*(\\d+)\\s*\\)$");
        std::smatch match;
    ...
    std::optional<SK_Color> parseRGBA(const std::string& color) {
        std::regex rgbaRegex("^rgba\\(\\s*(\\d+)\\s*,?\\s*(\\d+)\\s*,?\\s*(\\d+)\\s*,?\\s*([\\d.]+)\\s*\\)$");
        std::smatch match;
    ...
    std::optional<SK_Color> parseHSL(const std::string& color) {
        std::regex hslRegex("^hsl\\(\\s*(-?[\\d.]+)\\s*(?:deg)?\\s*,?\\s*([\\d.]+)%\\s*,?\\s*([\\d.]+)%\\s*\\)$");
        std::smatch match;
    ...
    std::optional<SK_Color> parseHSLA(const std::string& color) {
        std::regex hslaRegex("^hsla\\(\\s*(-?[\\d.]+)\\s*(?:deg)?\\s*,?\\s*([\\d.]+)%\\s*,?\\s*([\\d.]+)%\\s*,?\\s*([\\d.]+)\\s*\\)$");
        std::smatch match;
...
static inline void SK_Colors_Init() {
    SK_Colors::list.insert({
