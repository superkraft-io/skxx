...
class sk_dawPluginMngr {
    ...
    startReadMonitor() {
        var step = async _ts => {
            ...
            for (var id in this.components) {
                ...
                if (!component.dawPluginParamInfo.busyChanging)
    ...
    static configRootEvents(target) {
        ...
        target.dawPluginParamInfo.writeValue = async (opt = {}) => {
            if (target.dawPluginParamInfo.blockWrite || target.dawPluginParamInfo.busyReading) return;
        ...
        target.dawPluginParamInfo.readValue = async (opt = {}) => {
            ...
            try {
                ...
                var res = sk_api.pluginMngr.parameterByID(target.pluginParamID)
                ...
                if (!target.dawPluginParamInfo.busyWriting || !target.dawPluginParamInfo.busyChanging){
                    if (sk_api.pluginMngr.onParameterRead) sk_api.pluginMngr.onParameterRead(target, opt, res)
